...
from civil_registration_automation.constants.constants import (
    INVALID_TEST_DATA,
    HTTP_STATUS,
    RESPONSE_FIELDS,
)
from civil_registration_automation.pages.get_civil_api import (
    get_all_civil_records,
    get_civil_record_by_id,
    get_civil_record_by_invalid_get_endpoint,
)
...
from civil_registration_automation.utils.civil_record_builder import (
    build_civil_record_data,
)
...
def test_get_all_civil_record(created_record):
    ...
    assert (
        response.status_code == HTTP_STATUS["OK"]
    ), f"Get all records failed: {response.status_code} {response.text}"
    ...
    ), f"Expected to find created record with ID {created_record['id']} in the list, but it was missing!"
...
def test_get_civil_record_by_id(created_record):
    ...
    assert (
        response.status_code == HTTP_STATUS["OK"]
    ), f"Get record by ID failed: {response.status_code} {response.text}"
...
def test_get_civil_record_by_invalid_get_endpoint():
    ...
    ), f"Expected {HTTP_STATUS['NOT_FOUND']} for invalid endpoint, but got {response.status_code}"
...
def test_get_civil_with_nonexistent_id(cleanup_box):
    ...
    assert (
        response.status_code == HTTP_STATUS["NOT_FOUND"]
    ), f"Expected {HTTP_STATUS['NOT_FOUND']} for nonexistent ID, but got {response.status_code}"
...
def test_get_civil_with_invalid_id_format(created_record, cleanup_box):
    ...
    ), f"Expected {HTTP_STATUS['BAD_REQUEST']} for invalid ID format, but got {response.status_code}"
